...
public sealed class GrevDadSaveSyncService : IDisposable
{
    ...
    public async Task<CloudSaveState> DownloadAsync(string grevId, string appId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                return new CloudSaveState(CloudSaveStatus.Error, manifest.LastUploadedAtUtc,
                    manifest.LastDownloadedAtUtc, "The cloud save exceeds Grev Home's 300 MB download limit.");
            }
            ...
            try
            {
                ...
                if (Directory.Exists(saveRoot) && Directory.EnumerateFileSystemEntries(saveRoot).Any())
                {
                    backupRoot = $"{saveRoot}.backup-{DateTime.Now:yyyyMMdd-HHmmss-fff}-{Guid.NewGuid():N}";
                    Directory.Move(saveRoot, backupRoot);
                }
                else if (Directory.Exists(saveRoot))
                {
                    Directory.Delete(saveRoot);
                }
                try
                {
                ...
                {
                    if (backupRoot is not null && Directory.Exists(backupRoot) && !Directory.Exists(saveRoot))
                        Directory.Move(backupRoot, saveRoot);
                    throw;
                }
            }
            finally
    ...
    public async Task<CloudSaveState> CheckRemoteAsync(string grevId, string appId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var saveRoot = _paths.GetProfileAppSaves(grevId, appId);
            var hasLocalData = Directory.Exists(saveRoot) && Directory.EnumerateFileSystemEntries(saveRoot).Any();
            var localChanged = manifest.LastUploadedHash is null
                ? hasLocalData
                : !string.Equals(currentHash, manifest.LastUploadedHash, StringComparison.Ordinal);
            ...
            var remoteUpdatedAt = TryReadRemoteUpdatedAtHeader(response);
            // A remote save on a device that has never synced locally is remote work, not a blank
            // baseline. This prevents a fresh install from blindly replacing an existing cloud save.
            var remoteChanged = remoteUpdatedAt is not null &&
                                (manifest.LastKnownRemoteUpdatedAtUtc is null ||
    ...
    private static async Task CopyWithLimitAsync(HttpContent content, Stream destination, long limit, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            if (read == 0) break;
            total += read;
            if (total > limit) throw new InvalidDataException("The cloud save exceeds Grev Home's 300 MB download limit.");
    ...
    public static void ValidateArchive(string archivePath)
    {
        ...
        if (archive.Entries.Count > MaxArchiveEntries)
            throw new InvalidDataException("The cloud save contains too many files.");

        long expandedBytes = 0;
        foreach (var entry in archive.Entries)
        {
            if (entry.Length < 0 || entry.Length > MaxExtractedBytes - expandedBytes)
                throw new InvalidDataException("The expanded cloud save exceeds Grev Home's 1 GB safety limit.");
