...
public sealed partial class GrevDadCoordinator
{
    ...
    private async Task UploadSaveIfEnabledSafeAsync(GrevDadSaveSyncService sync, string grevId, string appId, string appName)
    {
        try
        {
            ...
            // CheckRemoteAsync is a network call this automatic path would otherwise not need; a
            // failure or timeout here is treated the same as "no conflict detected" and falls
            // through to the ordinary blind upload, matching this path's existing best-effort
            // contract rather than turning a slow/unreachable conflict check into a skipped sync.
            ...
            {
                remote = new CloudSaveState(CloudSaveStatus.LocalChangesPending, null, null, null);
            }
