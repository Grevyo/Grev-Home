...
When Grev Home launches a verified release with `--update`, the custom installer enters update mode automatically. It waits for the existing shell to close, runs the embedded engine silently, preserves runtime-created profiles, games, artwork and settings, then reopens the updated Grev Home shell. A failed or timed-out update remains on a recoverable Try again screen instead of abandoning the downloaded installer.
